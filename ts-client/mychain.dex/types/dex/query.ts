...
export class QueryClientImpl implements Query {
  ...
  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("mychain.dex.Query", "Params", data);
    return promise.then((data) => QueryParamsResponse.decode(new _m0.Reader(data)));
  }

  Pool(request: QueryGetPoolRequest): Promise<QueryGetPoolResponse> {
    const data = QueryGetPoolRequest.encode(request).finish();
    const promise = this.rpc.request("mychain.dex.Query", "Pool", data);
  ...
  PoolAll(request: QueryAllPoolRequest): Promise<QueryAllPoolResponse> {
    ...
    const promise = this.rpc.request("mychain.dex.Query", "PoolAll", data);
